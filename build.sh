...
case "$COMMAND" in
  ...
  semver-check)
    ...
    OLD_SO="$OLD_DIR/build/liba_json_schema_builder_library_shared.so"
    NEW_SO="build/liba_json_schema_builder_library_shared.so"
  ...
  coverage)
    ...
    llvm-cov report "$COV_BUILD_DIR/liba_json_schema_builder_library_debug.a" \
